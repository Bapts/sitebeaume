...
function updateLanguage(localisation) {
    function loadJSON(callback) {

        var xobj = new XMLHttpRequest();
        var file = '../language/' + localisation + '.json';
    ...
    $(document).ready(function () {
        ...
        loadJSON(function (response) {
            ...
            function ShowResults(value, index, ar) {
                ...
                if (value.innerText != undefined && value.innerText.length > 0) {
                    var trad = actual_JSON[window.location.pathname][value.innerText.toLowerCase()]
                    if (trad != undefined) {
                        console.log("trad = " + trad);
                        value.innerHTML = value.innerHTML.replace(value.innerText.toLowerCase(), trad);
                    }
                }
            }
            var div = document.getElementsByClassName("text");
            var divList = Array.prototype.slice.call(div);
            divList.forEach(ShowResults);
